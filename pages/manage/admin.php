...
function handleAdd($conn, $type, $data) {
    ...
    switch($type) {
        case 'computer':
            $sql = "INSERT INTO computer (Com_name, IPaddress, Branch_ID, Department_ID, Stat_ID) VALUES (?, ?, ?, ?, ?)";
            break;
        case 'monitor':
            $sql = "INSERT INTO monitor (Monitor_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'mouse':
            $sql = "INSERT INTO mouse (Mouse_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'keyboard':
            $sql = "INSERT INTO keyboard (Key_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'powersup':
            $sql = "INSERT INTO powersup (Power_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'wifi':
            $sql = "INSERT INTO wifi (Wifi_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'printer':
            $sql = "INSERT INTO printer (Printer_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'tablet':
            $sql = "INSERT INTO tablet (Tablet_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'switch':
            $sql = "INSERT INTO switch_hub (Switch_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'flashdrive':
            $sql = "INSERT INTO flashdrive (Flash_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'harddisk':
            $sql = "INSERT INTO harddisk (HDD_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'speaker':
            $sql = "INSERT INTO speaker (Speaker_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'adapter':
            $sql = "INSERT INTO adapter (Adapter_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
        case 'notebook':
            $sql = "INSERT INTO notebook (NB_SN, Com_ID, Stat_ID) VALUES (?, ?, ?)";
            break;
    }
    ...
    if ($sql) {
        ...
        switch($type) {
            case 'computer':
                $stmt->bind_param("ssiii", $data['name'], $data['ip'], $data['branch'], $data['department'], $data['status']);
                break;
            case 'monitor':
            case 'mouse':
            case 'keyboard':
            ...
            case 'adapter':
            case 'notebook':
                $stmt->bind_param("sii", $data['sn'], $data['com_id'], $data['status']);
                break;
        }
        return $stmt->execute();
    }
...
function handleDelete($conn, $type, $id) {
    ...
    switch($type) {
        ...
        case 'tablet':
            ...
            break;
        case 'switch':
            $sql = "DELETE FROM switch_hub WHERE Switch_ID = ?";
            break;
        case 'flashdrive':
...
function handleEdit($conn, $type, $data) {
    ...
    switch($type) {
        case 'computer':
            $sql = "UPDATE computer SET Com_name = ?, IPaddress = ?, Branch_ID = ?, Department_ID = ?, Stat_ID = ? WHERE Com_ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssiiii", $data['name'], $data['ip'], $data['branch'], $data['department'], $data['status'], $data['id']);
            break;
        case 'monitor':
            $sql = "UPDATE monitor SET Monitor_SN = ?, Com_ID = ?, Stat_ID = ? WHERE Monitor_ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("siii", $data['sn'], $data['com_id'], $data['status'], $data['id']);
            break;
        case 'switch':
            $sql = "UPDATE switch_hub SET Switch_SN = ?, Com_ID = ?, Stat_ID = ? WHERE Switch_ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("siii", $data['sn'], $data['com_id'], $data['status'], $data['id']);
            break;
    }
