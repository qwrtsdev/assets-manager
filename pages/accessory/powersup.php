...
$current_page = 'powersup';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT up.*, up.StatusID as StatusNumber, s.StatusName
            FROM ups up
            JOIN status s ON up.StatusID = s.StatusID
            WHERE up.UpsID LIKE ? 
            OR up.UpsSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | เครื่องสำรองไฟ</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-plug"></i> ข้อมูลเครื่องสำรองไฟ</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["UpsSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=6&id=" . htmlspecialchars($row["UpsID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=6&id=" . htmlspecialchars($row["UpsID"]) . "' class='btn btn-info btn-sm'>
