...
$sql = "    SELECT ip.*, ip.StatusID as StatusNumber, s.StatusName
            FROM ipad ip
            JOIN status s ON ip.StatusID = s.StatusID
            WHERE ip.iPadID LIKE ? 
            OR ip.iPadSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | iPad & Tablet</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-tablet-alt"></i> ข้อมูล iPad & Tablet</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["iPadSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=12&id=" . htmlspecialchars($row["iPadID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=12&id=" . htmlspecialchars($row["iPadID"]) . "' class='btn btn-info btn-sm'>
