...
$current_page = 'adapter';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT ad.*, ad.StatusID as StatusNumber, s.StatusName
            FROM adapter ad
            JOIN status s ON ad.StatusID = s.StatusID
            WHERE ad.AdapterID LIKE ? 
            OR ad.AdapterSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | อะแดปเตอร์และสายชาร์จ</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-charging-station"></i> ข้อมูลอะแดปเตอร์และสายชาร์จ</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["AdapterSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=7&id=" . htmlspecialchars($row["AdapterID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=7&id=" . htmlspecialchars($row["AdapterID"]) . "' class='btn btn-info btn-sm'>
