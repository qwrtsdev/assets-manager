...
$sql = "    SELECT r.*, r.StatusID as StatusNumber, s.StatusName
            FROM router r
            JOIN status s ON r.StatusID = s.StatusID
            WHERE r.RouterID LIKE ? 
            OR r.RouterSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | เราต์เตอร์</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-wifi"></i> ข้อมูลเราต์เตอร์</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["RouterSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=8&id=" . htmlspecialchars($row["RouterID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=8&id=" . htmlspecialchars($row["RouterID"]) . "' class='btn btn-info btn-sm'>
