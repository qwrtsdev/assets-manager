...
$sql = "    SELECT sw.*, sw.StatusID as StatusNumber, s.StatusName
            FROM switchhub sw
            JOIN status s ON sw.StatusID = s.StatusID
            WHERE sw.SwitchHubID LIKE ? 
            OR sw.SwitchHubSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | สวิตช์ฮับ</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-network-wired"></i> ข้อมูลสวิตช์ฮับ</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["SwitchHubSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=9&id=" . htmlspecialchars($row["SwitchHubID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=9&id=" . htmlspecialchars($row["SwitchHubID"]) . "' class='btn btn-info btn-sm'>
