...
$sql = "    SELECT m.*, s.StatusID as StatusNumber, s.StatusName
            FROM monitor m
            JOIN status s ON m.StatusID = s.StatusID
            WHERE m.MonitorID LIKE ? 
            OR m.MonitorSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | จอภาพ</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-tv"></i> ข้อมูลจอภาพ</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["MonitorSN"]) . "</td>";
                            ...
                            echo "<td>" .
                                "<a href='/dbquery/pages/manage/repair-form.php?type=3&id=" . htmlspecialchars($row["MonitorID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=3&id=" . htmlspecialchars($row["MonitorID"]) . "' class='btn btn-info btn-sm'>
