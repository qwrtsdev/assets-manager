...
$current_page = 'flashdrive';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT us.*, us.StatusID as StatusNumber, s.StatusName
            FROM usb us
            JOIN status s ON us.StatusID = s.StatusID
            WHERE us.USBID LIKE ? 
            OR us.USBSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | แฟลชไดรฟ์</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fa-solid fa-link"></i> ข้อมูลแฟลชไดรฟ์</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["USBSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=10&id=" . htmlspecialchars($row["USBID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=10&id=" . htmlspecialchars($row["USBID"]) . "' class='btn btn-info btn-sm'>
