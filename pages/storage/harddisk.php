...
$current_page = 'harddisk';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT h.*, h.StatusID as StatusNumber, s.StatusName
            FROM harddisk h
            JOIN status s ON h.StatusID = s.StatusID
            WHERE h.HarddiskID LIKE ? 
            OR h.HarddiskSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | ฮาร์ดดิสก์</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-hdd"></i> ข้อมูลฮาร์ดดิสก์</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["HarddiskSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=16&id=" . htmlspecialchars($row["HarddiskID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=16&id=" . htmlspecialchars($row["HarddiskID"]) . "' class='btn btn-info btn-sm'>
