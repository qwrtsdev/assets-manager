...
$current_page = 'keyboard';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT k.*, s.StatusID as StatusNumber, s.StatusName
            FROM keyboard k
            JOIN status s ON k.StatusID = s.StatusID
            WHERE k.KeyboardID LIKE ? 
            OR k.KeyboardSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | คีย์บอร์ด</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-keyboard"></i> ข้อมูลคีย์บอร์ด</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["KeyboardSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=4&id=" . htmlspecialchars($row["KeyboardID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=4&id=" . htmlspecialchars($row["KeyboardID"]) . "' class='btn btn-info btn-sm'>
