...
$current_page = 'speaker';
$search = isset($_GET['search']) ? $_GET['search'] : null;

$sql = "    SELECT sp.*, sp.StatusID as StatusNumber, s.StatusName
            FROM speaker sp
            JOIN status s ON sp.StatusID = s.StatusID
            WHERE sp.SpeakerID LIKE ? 
            OR sp.SpeakerSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | ลำโพง</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-volume-up"></i> ข้อมูลลำโพง</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["SpeakerSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=11&id=" . htmlspecialchars($row["SpeakerID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=11&id=" . htmlspecialchars($row["SpeakerID"]) . "' class='btn btn-info btn-sm'>
