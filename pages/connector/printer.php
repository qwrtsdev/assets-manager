...
$sql = "    SELECT p.*, s.StatusID as StatusNumber, s.StatusName
            FROM printer p
            JOIN status s ON p.StatusID = s.StatusID
            WHERE p.PrinterID LIKE ? 
            OR p.PrinterSN LIKE ?  ";
...
<head>
    ...
    <title>ระบบจัดการสินทรัพย์ | เครื่องพิมพ์</title>
...
<body>
    ...
    <div class="container-lg w-100 px-3 my-5 mx-auto">
        <h1 class="page-title"><i class="fas fa-print"></i> ข้อมูลเครื่องพิมพ์</h1>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>";
                            echo "<td>" . $counter++ . "</td>";
                            echo "<td>" . htmlspecialchars($row["PrinterSN"]) . "</td>";
                            ...
                            echo "<td>
                                <a href='/dbquery/pages/manage/repair-form.php?type=13&id=" . htmlspecialchars($row["PrinterID"]) . "' class='btn btn-warning btn-sm'>
                                    <i class='fas fa-tools'></i> บันทึกการซ่อม
                                </a>
                                <a href='/dbquery/pages/manage/assets-edit.php?type=13&id=" . htmlspecialchars($row["PrinterID"]) . "' class='btn btn-info btn-sm'>
