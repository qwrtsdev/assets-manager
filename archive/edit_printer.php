...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_stmt = $conn->prepare("UPDATE printer SET 
                Printer_SN=?, 
                Com_ID=?, 
                Stat_ID=?, 
                Branch_ID=?, 
                ...
                DateAdd=? 
            WHERE Printer_ID=?");
    $update_stmt->bind_param("ssssssssss", $printer_sn, $com_id, $stat_id, $branch_id, $department_id, $user_resp, $short_name_resp, $detail, $date_add, $printer_id);
    $update_stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>แก้ไขข้อมูลเครื่องพิมพ์</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css">
</head>
<body>
<div class="container">
    <h1>แก้ไขข้อมูลเครื่องพิมพ์</h1>
    <form method="POST" action="">
        <input type="hidden" name="printer_id" value="<?php echo htmlspecialchars($printer['Printer_ID']); ?>">
        <div class="mb-3">
            <label for="printer_sn" class="form-label">หมายเลขซีเรียล:</label>
            <input type="text" class="form-control" id="printer_sn" name="printer_sn" 
                   value="<?php echo htmlspecialchars($printer['Printer_SN']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="com_id" class="form-label">รหัสคอมพิวเตอร์:</label>
            <input type="text" class="form-control" id="com_id" name="com_id" 
                   value="<?php echo htmlspecialchars($printer['Com_ID']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="stat_id" class="form-label">สถานะ</label>
            <select class="form-control" id="stat_id" name="stat_id" required>
                <?php while ($status = $statuses->fetch_assoc()): ?>
                    <option value="<?php echo $status['Stat_ID']; ?>" <?php echo ($status['Stat_ID'] == $printer['Stat_ID']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="Branch_ID" name="Branch_ID" required>
                <?php while ($branch = $branches->fetch_assoc()): ?>
                    <option value="<?php echo $branch['Branch_ID']; ?>" <?php echo ($branch['Branch_ID'] == $printer['Branch_ID']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="Department_ID" name="Department_ID" required>
                <?php while ($department = $departments->fetch_assoc()): ?>
                    <option value="<?php echo $department['Department_ID']; ?>" <?php echo ($department['Department_ID'] == $printer['Department_ID']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="user_resp" name="user_resp" 
                   value="<?php echo htmlspecialchars($printer['UserResp']); ?>" required>
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="short_name_resp" name="short_name_resp" 
                   value="<?php echo htmlspecialchars($printer['ShortNameResp']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="detail" class="form-label">รายละเอียด:</label>
            <textarea class="form-control" id="detail" name="detail" required><?php echo htmlspecialchars($printer['Detail']); ?></textarea>
        </div>
        <div class="mb-3">
            <label for="date_add" class="form-label">วันที่เพิ่ม:</label>
            <input type="date" class="form-control" id="date_add" name="date_add" 
                   value="<?php echo htmlspecialchars(date('Y-m-d', strtotime($printer['DateAdd']))); ?>" required>
        </div>
