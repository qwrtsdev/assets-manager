...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_stmt = $conn->prepare("UPDATE monitor SET 
                Monitor_SN=?, 
                Com_ID=?, 
                Stat_ID=?, 
                Branch_ID=?, 
                ...
                DateAdd=? 
            WHERE Monitor_ID=?");
    $update_stmt->bind_param("ssssssssss", $monitor_sn, $com_id, $stat_id, $branch_id, $department_id, $user_resp, $short_name_resp, $detail, $date_add, $monitor_id);
    $update_stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>แก้ไขข้อมูลจอภาพ</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css">
</head>
<body>
<div class="container">
    <h1>แก้ไขข้อมูลจอภาพ</h1>
    <form method="POST" action="">
        <input type="hidden" name="monitor_id" value="<?php echo htmlspecialchars($monitor['Monitor_ID']); ?>">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="Branch_ID" name="Branch_ID" required>
                <?php while ($branch = $branches->fetch_assoc()): ?>
                    <option value="<?php echo $branch['Branch_ID']; ?>" <?php echo ($branch['Branch_ID'] == $monitor['Branch_ID']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="Department_ID" name="Department_ID" required>
                <?php while ($department = $departments->fetch_assoc()): ?>
                    <option value="<?php echo $department['Department_ID']; ?>" <?php echo ($department['Department_ID'] == $monitor['Department_ID']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="user_resp" name="user_resp" 
                   value="<?php echo htmlspecialchars($monitor['UserResp']); ?>" required>
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="short_name_resp" name="short_name_resp" 
                   value="<?php echo htmlspecialchars($monitor['ShortNameResp']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="detail" class="form-label">รายละเอียด:</label>
            <textarea class="form-control" id="detail" name="detail" required><?php echo htmlspecialchars($monitor['Detail']); ?></textarea>
        </div>
        <div class="mb-3">
            <label for="date_add" class="form-label">วันที่เพิ่ม:</label>
            <input type="date" class="form-control" id="date_add" name="date_add" 
                   value="<?php echo htmlspecialchars(date('Y-m-d', strtotime($monitor['DateAdd']))); ?>" required>
        </div>
